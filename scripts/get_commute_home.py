...
destination = os.getenv("HOME_ADDR")
origin = os.getenv("WORK_ADDR")
...
# 4. Save to CSV
with open('./datasets/commute_home_data.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(["Time", "Duration_Minutes"])
    writer.writerows(results)

print("Done! Data saved to commute_home_data.csv")